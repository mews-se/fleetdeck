...
const FD = (() => {
  ...
  // a mismatch is red; a bond answering from its other port is only noted
  const arpPill = (x) => x.arp_mac && x.mac && x.arp_mac !== x.mac ? `<br>${pill(x.mismatch ? 'crit' : 'off', 'ARP ' + x.arp_mac)}` : '';
  ...
  R.hosts = (d) => {
    ...
    $('devices').innerHTML = (d.devices || []).map((g) => `<div class="panel" style="margin-top:16px">
      ...
      ${g.devices.length ? `<div class="tw"><table><tr><th></th><th>Address</th><th>MAC</th><th>Name</th><th>Description</th><th>DHCP</th><th>Lease ends</th></tr>${g.devices.map((x) => `<tr><td>${dot(x.online ? 'good' : 'off')}</td><td class="mono">${esc(x.ip)}</td><td class="mono small">${esc(x.mac || '')}${arpPill(x)}</td><td>${esc(x.hostname || '')}</td><td class="wrap">${esc(x.descr || '')}</td><td>${pill(dhcpKind[x.kind], x.kind)}</td><td class="dim">${x.ends ? when(x.ends) : '—'}</td></tr>`).join('')}</table></div>` : '<div class="empty">Nothing read from pfSense yet.</div>'}
    </div>`).join('');
  ...
  R.network = (d) => {
    ...
    $('pf').innerHTML = (d.pfsense || []).map((p) => {
      ...
      const devs = p.devices.length ? `<div class="tw" style="margin-top:8px"><table><tr><th></th><th>Address</th><th>MAC</th><th>Name</th><th>Description</th><th>DHCP</th><th>Lease ends</th><th>fleetdeck</th></tr>${p.devices.map((x) => `<tr${x.quiet ? ' class="dim"' : ''}><td>${dot(x.online ? 'good' : 'off')}</td><td class="mono">${esc(x.ip)}</td><td class="mono small">${esc(x.mac || '')}${arpPill(x)}</td><td>${esc(x.hostname || '')}</td><td class="wrap">${esc(x.descr || '')}</td><td>${pill(dhcpKind[x.kind], x.kind === 'arp' ? 'arp only' : x.kind)}${x.quiet ? ' ' + pill('off', 'quiet') : ''}</td><td class="dim">${x.ends ? when(x.ends) : '—'}</td><td>${x.host ? `<a href="/hosts/${esc(x.host)}">${esc(x.host)}</a>` : ''}</td></tr>`).join('')}</table></div>` : '<div class="empty">No leases read yet.</div>';
  ...
  const actionRow = (a) => `<tr data-action="${esc(a.id)}"><td class="wrap"><b>${esc(a.title)}</b><br><span class="small">${esc(a.summary)}</span>${a.params.map((p) => ` <label class="small">${esc(p.name)} <input data-param="${esc(p.name)}" value="${esc(p.default)}" pattern="${esc(p.pattern)}"></label>`).join('')}${a.note ? `<br><span class="dim">${esc(a.note)}</span>` : ''}</td><td class="sub">${targetCell(a)}</td><td>${pill(a.policy, a.policy)}</td><td class="num dim">${a.last ? `<a href="/actions/runs/${a.last.id}" title="exit ${a.last.exit}">${age(a.last.ts)}</a>${a.last.exit === 0 ? '' : a.last.exit == null ? ' ⋯' : ' ✗'}` : '—'}</td><td class="r"><button class="btn sm ${a.policy === 'free' ? 'primary' : ''}" data-run="${esc(a.id)}">${a.policy === 'confirm' ? 'Run…' : 'Run'}</button></td></tr>`;
  ...
  R.host = (d) => {
    ...
    if (n) {
      const x = n.device;
      kv += `<dt>DHCP</dt><dd>${x ? `${dhcpPill({ mapping: x.kind, mismatch: x.mismatch })} <span class="mono">${esc(x.mac || '')}</span>${x.arp_mac && x.mac && x.arp_mac !== x.mac ? ` · ARP says <span class="mono">${esc(x.arp_mac)}</span>` : ''} · ${x.online ? 'seen in ARP' : 'not in ARP'}${x.hostname ? ` · ${esc(x.hostname)}` : ''}${x.ends ? ` · until ${when(x.ends)}` : ''}` : `no mapping or lease on ${esc(n.box)}`} · <a href="/network">network</a></dd>`;
    }
    $('sys-kv').innerHTML = kv;
    if (d.pfsense) {
